...
export class QuickbookDesktopComponent implements OnInit {
    ...
    constructor(public router: Router, private company_service: CompanyService, private auth_service: AuthService, private sanitizer: DomSanitizer) {
        ...
        this.company_service.getCompanyMetadata().subscribe(
            data => {
                if (data.qb_desktop_installed == true) {
                    ...
                    this.qbd_app_uri = this.sanitizer.bypassSecurityTrustUrl('espresso:'+this.user_data.username+','+data.monthly_reporting_current_period);
                }
